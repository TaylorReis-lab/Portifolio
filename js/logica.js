;(function () {
  ...
  if (btnSayHello) {
    btnSayHello.addEventListener('click', () => {
      openExternal('https://signal.me/#eu/_hU56h7yNME5tlOoPByNRLWzJVUk14FTQU__WybpxDK1tfNmqTJEprLpxWy_Q_OG')
    })
  }
  ...
  const modalPageIndicator = document.getElementById('projects-modal-page-indicator')
  ...
  function inferTagsAndLegend(repo) {
    ...
    const lower = ((repo.name || '') + ' ' + (repo.description || '')).toLowerCase()
    if (lower.includes('bot')) tags.push('Bot')
    if (lower.includes('chat')) tags.push('Chat')
    if (lower.includes('api')) tags.push('API')
    if (lower.includes('ecommerce') || lower.includes('e-commerce') || lower.includes('shop')) tags.push('E-commerce')
    // dedupe
  ...
  async function fetchGithubProjects() {
    try {
      const resp = await fetch(`https://api.github.com/users/${GITHUB_USER}/repos?per_page=200&sort=pushed`)
      ...
      additionalProjects = repos
        ...
        .map(r => {
          ...
          const desc = r.description || (r.homepage ? 'Demo available' : 'No description provided')
      ...
      totalModalPages = Math.max(1, Math.ceil(additionalProjects.length / PAGE_SIZE))
    ...
    for (let i = 0; i < count; i++) {
      ...
      icons.forEach(icon => {
        ...
        newIcon.addEventListener('click', () => {
          ...
          if (name === 'link') openExternal(repo.link || repo.repo || 'https://example.com')
          if (name === 'code') openExternal(repo.repo || repo.link || 'https://github.com')
        })
      })
    }
  }
  ...
  function renderModalPage(page) {
    ...
    setTimeout(() => {
      ...
      modalGrid.querySelectorAll('.group').forEach((card, idx) => {
        ...
        icons.forEach(icon => {
          ...
          icon.addEventListener('click', () => {
            ...
            if (name === 'link') window.open(proj.link || 'https://example.com', '_blank')
            if (name === 'code') window.open(proj.repo || 'https://github.com', '_blank')
          })
        })
      ...
      modalPageIndicator && (modalPageIndicator.textContent = `Page ${currentModalPage + 1} / ${totalModalPages}`)
    }, 160)
  }
  ...
  if (modalPagePrev) modalPagePrev.addEventListener('click', () => renderModalPage(currentModalPage - 1))
  if (modalPageNext) modalPageNext.addEventListener('click', () => renderModalPage(currentModalPage + 1))
  if (modalNavPrev) modalNavPrev.addEventListener('click', () => renderModalPage(currentModalPage - 1))
  if (modalNavNext) modalNavNext.addEventListener('click', () => renderModalPage(currentModalPage + 1))
